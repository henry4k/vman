...
class condition_variable {
  public:
    ...
    inline cv_status::_enum wait_for(_mutexT &aMutex, const chrono::duration<_Rep, _Period>& aTime)
    {
      const double seconds = double(aTime.count()) * _Period::_as_double();
#if defined(_TTHREAD_WIN32_)
      // Increment number of waiters
      EnterCriticalSection(&mWaitersCountLock);
      ++ mWaitersCount;
      ...
      // the number of waiters when done)...
      aMutex.unlock();
      const cv_status::_enum r = _wait(seconds);
      aMutex.lock();
      return r;
#else
      struct timespec waitTime;
      _makeWaitTime(&waitTime, seconds);
      if( pthread_cond_timedwait(&mHandle, &aMutex.mHandle, &waitTime) == ETIMEDOUT )
  ...
  private:
#if defined(_TTHREAD_WIN32_)
    cv_status::_enum _wait(const double seconds);
    HANDLE mEvents[2];                  ///< Signal and broadcast event HANDLEs.
    unsigned int mWaitersCount;         ///< Count of the number of waiters.
    CRITICAL_SECTION mWaitersCountLock; ///< Serialize access to mWaitersCount.
#else
    void _makeWaitTime(struct timespec* waitTime, double seconds);
